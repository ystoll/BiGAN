...
class BGAN_D(nn.Module):
    ...
    def merge_score(self, score_f, score_b):
        ...
        # Calculate Loss for Linear layer
        scores_real = (
            (score_f["scoresReg"] + score_b["scoresReg"]) * score_b["missing"]
        ) / 2
        scores_real = torch.sum(scores_real) / (torch.sum(score_b["missing"]) + 1e-5)

        scores_fake = (
            (score_f["scoresReg"] + score_b["scoresReg"]) * (1 - score_b["missing"])
        ) / 2
        scores_fake = torch.sum(scores_fake) / (
            torch.sum(1 - score_b["missing"]) + 1e-5
        )

        loss_dReg = -scores_real + scores_fake

        loss_gReg = -scores_fake
        ...
        # print(loss_gF,loss_gB)
        loss_gSig = loss_gF + loss_gB

        loss_dReal = adversarial_loss(
            (score_f["scoresSig"])[real_ids], valid
        ) + adversarial_loss((score_b["scoresSig"])[real_ids], valid)
        loss_dFake = adversarial_loss(
            (score_f["scoresSig"])[fake_ids], fake
        ) + adversarial_loss((score_b["scoresSig"])[fake_ids], fake)
        loss_dSig = (loss_dReal + loss_dFake) / 2

        return {"loss_d": loss_dReg + loss_dSig, "loss_g": loss_gReg + loss_gSig}
...
def run_on_batch(
    model, discriminator, data, mask, decay, rdecay, args, optimizer, optimizer_d, epoch
):
